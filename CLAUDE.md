...
- **Overview tab refetch**: uses `useFocusEffect` to call `refetch()` on `monthly-summary` + `daily-totals` every time the tab gains focus. All expense mutations call `refetchQueries(["monthly-summary"])` AND `refetchQueries(["daily-totals"])` (not `invalidateQueries` — that only marks stale; `refetchQueries` forces immediate background fetch)
- **gifted-charts BarChart data updates**: the library silently ignores `data` prop changes after initial mount. Always pass `key={barData.map(d => d.value).join('|')}` so React remounts the chart when values change. Without this, charts are frozen after edits.
- **gifted-charts x-axis line**: cannot be width-constrained without a ScrollView — it fills its container regardless of the `width` prop. Use `xAxisThickness={0}` to suppress it when chart fills a fixed-width card. Use `disableScroll` to prevent the internal gifted-charts ScrollView from accepting gestures.
- **Padding standard**: all card row content, list items, and inline dropdown rows use `paddingHorizontal: 14`. All tab screen scroll content uses `paddingHorizontal: 20` (outer margin). These must not be mixed.
